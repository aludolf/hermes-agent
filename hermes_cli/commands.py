...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("jobs", "Show orchestrator jobs or one job by id", "Session",
               gateway_only=True, args_hint="[job_id]"),
    CommandDef("parse", "Parse a PDF job and generate a summary", "Session",
               gateway_only=True, args_hint="[job_id]"),
    CommandDef("publish_kb", "Promote a canonical candidate to Domains_KB", "Session",
               gateway_only=True, args_hint="<job_id>"),
    CommandDef("approve_contact", "Approve a pending contact (owner only)", "Session",
               gateway_only=True, args_hint="<telegram_id> [capabilities]"),
    CommandDef("block_contact", "Block a contact (owner only)", "Session",
               gateway_only=True, args_hint="<telegram_id>"),
    CommandDef("contacts", "List all contacts by role (owner only)", "Session",
               gateway_only=True),
    CommandDef("lists", "Show all active shared lists", "Session",
               gateway_only=True),
    CommandDef("list", "Show items in a specific shared list", "Session",
               gateway_only=True, args_hint="<nome>"),
    CommandDef("newlist", "Create a new shared list (owner only)", "Session",
               gateway_only=True, args_hint="<nome> [tipo]"),
    CommandDef("remind", "Create a reminder with Calendar sync (owner only)", "Session",
               gateway_only=True, args_hint="<texto> <quando>"),
    CommandDef("reminders", "Show pending reminders (owner only)", "Session",
               gateway_only=True),
    ...
    CommandDef("briefing", "Trigger morning briefing on demand (owner only)", "Session",
               gateway_only=True),
    CommandDef("done", "Mark a list item as done/bought across all lists", "Session",
               gateway_only=True, args_hint="<item>"),
    CommandDef("recados", "Show pending errands (Recados list)", "Session",
               gateway_only=True),
    CommandDef("transcript",
               "Process pasted text as a transcript (owner only, preview-before-execute)",
               "Session",
               gateway_only=True, args_hint="<text>"),
    CommandDef("entity", "Show accumulated context for an entity (owner only)",
               "Session", gateway_only=True, args_hint="<name>"),
    CommandDef("entities", "List known entities with mention counts (owner only)",
               "Session", gateway_only=True),
    CommandDef("run_test", "Run a harness scenario against a target bot (owner only)",
               "Session", gateway_only=True, args_hint="<scenario_id>"),
    CommandDef("scenarios", "List available harness scenarios (owner only)",
               "Session", gateway_only=True),
    CommandDef("test_history", "Show recent harness runs (owner only)",
               "Session", gateway_only=True, args_hint="[scenario_id]"),
    # 022 Teams sentinel (owner-only)
    CommandDef("teams_connect", "Start Microsoft Teams OAuth device-code auth (owner only)",
               "Session", gateway_only=True),
    CommandDef("teams_watch", "Watch a Teams channel or chat (owner only)",
               "Session", gateway_only=True, args_hint="<resource_id> [alias]"),
    CommandDef("teams_unwatch", "Stop watching a Teams resource (owner only)",
               "Session", gateway_only=True, args_hint="<watch_id>"),
    CommandDef("teams_list", "List active Teams watches (owner only)",
               "Session", gateway_only=True),
    # 022 Email sentinel (owner-only)
    CommandDef("email_connect", "Connect an IMAP mailbox (owner only)",
               "Session", gateway_only=True, args_hint="<alias> <imap_host> <user>"),
    CommandDef("email_list", "List connected mailboxes (owner only)",
               "Session", gateway_only=True),
    CommandDef("email_disconnect", "Disconnect a mailbox (owner only)",
               "Session", gateway_only=True, args_hint="<alias>"),
    CommandDef("email_watch", "Watch a mailbox folder for new messages (owner only)",
               "Session", gateway_only=True, args_hint="<alias> [folder]"),
    CommandDef("email_unwatch", "Stop watching a folder (owner only)",
               "Session", gateway_only=True, args_hint="<watch_id>"),
    CommandDef("email_backfill", "Start async mailbox backfill for KB accumulation (owner only)",
               "Session", gateway_only=True, args_hint="<alias> [--since YYYY-MM-DD]"),
    CommandDef("email_backfill_status", "Show backfill job progress (owner only)",
               "Session", gateway_only=True, args_hint="[alias]"),
    CommandDef("teams_mute", "Mute a Teams watch (owner only)",
               "Session", gateway_only=True, args_hint="<alias> [hours]"),
    CommandDef("teams_unmute", "Unmute a Teams watch (owner only)",
               "Session", gateway_only=True, args_hint="<alias>"),
    CommandDef("teams_quiet", "Set quiet hours for a Teams watch (owner only)",
               "Session", gateway_only=True, args_hint="<alias> <HH-HH>"),
